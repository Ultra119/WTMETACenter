...
class UnitsCsvTranslator:
    ...
    def _load(self, path: str) -> None:
        ...
        if not os.path.exists(path):
            print("[UnitsCsv] ⚠️  units.csv недоступен — Rosetta Stone выключен, "
                  "VehicleDB будет использовать только fuzzy-матчинг")
            return
    ...
    def _try_download(self, dest: str) -> None:
        try:
            ...
            print(f"[UnitsCsv] 📥 Скачиваем units.csv из War-Thunder-Datamine …")
        ...
        try:
            with open(path, encoding="utf-8", errors="replace", newline="") as f:
                ...
                for i, row in enumerate(reader):
                    ...
                    if en_key not in self._en_to_id:
